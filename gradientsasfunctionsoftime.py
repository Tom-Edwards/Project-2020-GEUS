...
#was analysed
simulation_inf = {'matlab_output_folder': 'random_perbs/2012EXPprofile_1989-2019_100layers', 'initial_density_profile': '2012_Site-J_composite profile', 'number_of_layers': '100layers', 'time_period': '1989-2019'}
...
# =============================================================================
# 
# plt.plot([datetime.timedelta(hours=x*time_scaling).days for x in range(Number_of_time_points)], grad_RMSD_10cm, label='spacingc 10cm' )
# plt.plot([datetime.timedelta(hours=x*time_scaling).days for x in range(Number_of_time_points)], grad_RMSD_20cm, label='spacingc 20cm' )
# plt.plot([datetime.timedelta(hours=x*time_scaling).days for x in range(Number_of_time_points)], grad_RMSD_50cm, label='spacingc 50cm' )
# plt.plot([datetime.timedelta(hours=x*time_scaling).days for x in range(Number_of_time_points)], grad_RMSD_100cm, label='spacingc 100cm' )
# plt.plot([datetime.timedelta(hours=x*time_scaling).days for x in range(Number_of_time_points)], grad_RMSD_190cm, label='spacingc 190cm' )
# plt.legend()
# plt.savefig(path_to_outputs+'squaredifferenceofgradients.png')
# plt.close()
# 
# =============================================================================
...
# 
# 
# plt.plot([datetime.timedelta(hours=x*time_scaling).days for x in range(Number_of_time_points)], grad_10m_15m, label= r"$\Delta \rho = \rho_{(10m,15m]}-\rho_{[1m,10m]}$")
# plt.axvline(x=datetime.timedelta(hours=(target_hr)).days, linestyle=':', color='grey', label='Date: '+str(datetime.date(1989,1,1)+datetime.timedelta(hours=target_hr)))
# plt.axvline(x=datetime.timedelta(hours=(target_hr+8050)).days, linestyle='--', color='grey', label='Date: '+str(datetime.date(1989,1,1)+datetime.timedelta(hours=target_hr+8050)))
# plt.legend()
# plt.title(str(initial_density_profile)+ ' profile of negative gradients')
# plt.ylabel(r"Difference in mean density (${kg/m^3})$")
# plt.xlabel(r'Years from '+str(start_date)+ ' to '+ str(datetime.timedelta(hours=Number_of_time_points*time_scaling)+start_date))
# plt.savefig(path_to_outputs+'gradient_10m_15m'+str(initial_density_profile)+'.png')
# plt.close()
# 
# 
# plt.plot([datetime.timedelta(hours=x*time_scaling).days for x in range(Number_of_time_points)], grad_5m_15m, label= r"$\Delta = \rho_{(5m,15m]}-\rho_{[1m,5m]}$")
# plt.axvline(x=datetime.timedelta(hours=(target_hr)).days, linestyle=':', color='grey', label='Date: '+str(datetime.date(1989,1,1)+datetime.timedelta(hours=target_hr)))
# plt.axvline(x=datetime.timedelta(hours=(target_hr+8050)).days, linestyle='--', color='grey', label='Date: '+str(datetime.date(1989,1,1)+datetime.timedelta(hours=target_hr+8050)))
# plt.legend()
# plt.title(str(initial_density_profile)+ ' profile of negative gradients')
...
# 
# 
# plt.plot([datetime.timedelta(hours=x*time_scaling).days for x in range(Number_of_time_points)], interval_0m_1m, label= r"$rho_{[0,1]}$")
# plt.axvline(x=datetime.timedelta(hours=(target_hr)).days, linestyle=':', color='grey', label='Date: '+str(datetime.date(1989,1,1)+datetime.timedelta(hours=target_hr)))
# plt.axvline(x=datetime.timedelta(hours=(target_hr+8050)).days, linestyle='--', color='grey', label='Date: '+str(datetime.date(1989,1,1)+datetime.timedelta(hours=target_hr+8050)))
# plt.legend()
# plt.title(str(initial_density_profile)+ ' mean for interval (0,1]')
# plt.ylabel(r"mean density (${kg}/{m^3}$)")
# plt.xlabel(r'Years from '+str(start_date)+ ' to '+ str(datetime.timedelta(hours=Number_of_time_points*time_scaling)+start_date))
# plt.savefig(path_to_outputs+' mean for interval (0,1]'+str(initial_density_profile)+'.png')
# plt.close()
# 
# 
# plt.plot([datetime.timedelta(hours=x*time_scaling).days for x in range(Number_of_time_points)], interval_1m_10m, label= r"$rho_{(1,10]}$")
# plt.axvline(x=datetime.timedelta(hours=(target_hr)).days, linestyle=':', color='grey', label='Date: '+str(datetime.date(1989,1,1)+datetime.timedelta(hours=target_hr)))
# plt.axvline(x=datetime.timedelta(hours=(target_hr+8050)).days, linestyle='--', color='grey', label='Date: '+str(datetime.date(1989,1,1)+datetime.timedelta(hours=target_hr+8050)))
# plt.legend()
# plt.title(str(initial_density_profile)+ ' mean for interval (1,10]')
# plt.ylabel(r"mean density (${kg}/{m^3}$)")
# plt.xlabel(r'Years from '+str(start_date)+ ' to '+ str(datetime.timedelta(hours=Number_of_time_points*time_scaling)+start_date))
# plt.savefig(path_to_outputs+' mean for interval (1,10]'+str(initial_density_profile)+'.png')
# plt.close()
# 
# 
# plt.plot([datetime.timedelta(hours=x*time_scaling).days for x in range(Number_of_time_points)], interval_10m_15m, label= r"$rho_{(10,15]}$")
# plt.axvline(x=datetime.timedelta(hours=(target_hr)).days, linestyle=':', color='grey', label='Date: '+str(datetime.date(1989,1,1)+datetime.timedelta(hours=target_hr)))
# plt.axvline(x=datetime.timedelta(hours=(target_hr+8050)).days, linestyle='--', color='grey', label='Date: '+str(datetime.date(1989,1,1)+datetime.timedelta(hours=target_hr+8050)))
# plt.legend()
# plt.title(str(initial_density_profile)+ ' mean for interval [10,15]')
# plt.ylabel(r"mean density (${kg}/{m^3}$)")
# plt.xlabel(r'Years from '+str(start_date)+ ' to '+ str(datetime.timedelta(hours=Number_of_time_points*time_scaling)+start_date))
# plt.savefig(path_to_outputs+' mean for interval (10,15]'+str(initial_density_profile)+'.png')
# plt.close()
...
# plt.ylim(250,900)
# plt.legend(loc='upper left', prop={'size': 8})
# plt.title(str(initial_density_profile)+ ' mean for different intervals')
# plt.ylabel(r"mean density (${kg}/{m^3}$)")
# plt.xlabel(r'Years from '+str(start_date)+ ' to '+ str(datetime.timedelta(hours=Number_of_time_points*time_scaling)+start_date))
# plt.savefig(path_to_outputs+'intervals'+str(initial_density_profile)+'.png')
# plt.close()
